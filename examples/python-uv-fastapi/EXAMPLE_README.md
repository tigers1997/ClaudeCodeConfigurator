...
- **[`.claude/skills/`](.claude/skills/)** — thirteen skills: nine workflow commands (`/plan`, `/review-branch`, `/commit`, `/ship`, `/sync-docs`, `/check-context`, `/session-retro`, `/verify-setup`, `/retrofit`) plus the four micro-behavior toggles (`/careful`, `/freeze`, `/guard`, `/unfreeze`) enforced by `microbit-enforcer.sh`.
