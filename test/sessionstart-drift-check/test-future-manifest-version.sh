#!/usr/bin/env bash
# Manifest version > 2 → hook emits one-line "version mismatch" notice and exits 0.
set -euo pipefail
tmp=$(mktemp -d)
trap "rm -rf $tmp" EXIT
...
{ "manifest_version": 3, "written_at": "2026-05-13T00:00:00Z",
  "written_by": "cc-configure 9.9.9", "mcp_servers": ["git"] }
EOF
echo '{ "mcpServers": {} }' > "$tmp/.mcp.json"

out=$(CLAUDE_PROJECT_DIR="$tmp" bash templates/mcp/hooks/sessionstart-drift-check.sh 2>&1)
echo "$out" | grep -q "manifest version 3" \
    || { echo "FAIL: missing version note in output: '$out'"; exit 1; }
echo "$out" | grep -q "this hook is v2" \
    || { echo "FAIL: missing 'hook is v2' phrasing: '$out'"; exit 1; }
