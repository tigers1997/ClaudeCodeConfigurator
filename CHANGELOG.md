...
### Added
- **`docs/11-getting-started.md`** — two end-to-end walkthroughs that frame the configurator's place in a project's lifecycle. **New-project flow** prefixes scaffolding with `superpowers` brainstorming: `claude /plugin install superpowers` → `/brainstorm` (which hard-gates implementation until a design is approved) → capture to `docs/design.md` → `cc-configure` shaped by the design → install stack-specific plugins from the auto-generated `docs/recommended-plugins.md`. **Retrofit flow** documents the `--dry-run` → `cc-configure` → review `[ MERGED ]` / `[ COLLISIONS ]` → `/retrofit` skill → cleanup pattern. Plus daily-use mappings between configurator skills and their official-plugin equivalents, and triggers for re-running `cc-configure`. Linked from `00-overview.md` (now 10 numbered docs) and as a "first time?" call-out in the README's Use section.
- **Context-aware "Next steps" output** in `configure.py`. After scaffolding completes, the printed guidance branches on whether any `[ MERGED ]` or `[ COLLISIONS ]` entries fired (= retrofit) or not (= fresh project). New-project guidance prefixes the steps with the `superpowers` brainstorming bootstrap and points at `docs/recommended-plugins.md` for stack-specific install targets. Retrofit guidance points at `/retrofit` and `.claude-retrofit/` cleanup. Both branches link to `docs/11-getting-started.md`.

### Added
- **`recommend-plugins` module** (in default selection). Generates `docs/recommended-plugins.md` listing official Claude Code plugins to install alongside the scaffold. Static "always-recommended" block: `claude-code-setup`, `claude-md-management`, `feature-dev`, `commit-commands`, `superpowers` (with brainstorming flagged for new projects). Stack-specific block computed from form answers via new `compute_recommended_plugins()` helper: language → LSP plugin (`pyright-lsp` / `typescript-lsp` / `gopls-lsp` / `rust-analyzer-lsp` / etc.); database → DB plugin (`mongodb` / `neon` / `prisma` / `cloud-sql-postgresql` / `supabase` / `cockroachdb` / `pinecone`); framework → framework-specific plugin (`frontend-design` / `vercel` / `expo` / `rails-query` / `laravel-boost` / `shopify-ai-toolkit`); MCP toggles → official replacements (`github`, `playwright`, `context7`); deployment → cloud plugin (`aws-serverless` / `azure`); observability → APM plugin (`sentry` / `datadog` / `logfire`). Refreshes on every `cc-configure` run. New `target_path_for` route: `recommend-plugins/recommended-plugins.md` → `docs/recommended-plugins.md`. New placeholders: `{{recommended_plugins}}` and `{{generation_date}}`.
