...
### Added
- **Retrofit safety — Tier 3: `/retrofit` skill + retrofit guide doc.** Capstone of the retrofit-safety roadmap. The `/retrofit` skill (shipped in `commands-core`) reads `.claude-retrofit/REPORT.md`, parses the four section types (Deep-merged / Skipped / Renamed / Overwritten), and walks the user through each Skipped staging interactively: prints the diff, offers five choices (Keep yours / Replace yours with ours / Merge sections / Install ours alongside as `-cc` sibling / Skip for now), applies the user's decision with `.bak-retrofit-<date>` backups before any destructive op, and removes handled stagings from `.claude-retrofit/incoming/`. Finishes with a per-file before→after summary and a reminder to commit. Backs the discipline with explicit rules: never auto-pick on the user's behalf; never apply a merge without showing it first; never delete `.claude-retrofit/` without confirmation; never overwrite without a backup. Edge cases handled (incoming exists but yours gone; both now identical; missing REPORT.md; structurally-incompatible merge — recommend rename).
- **`docs/09-retrofit-guide.md`** — the "what belongs in CLAUDE.md vs. elsewhere" triage doc. Categories: stays in CLAUDE.md (project-wide invariants, commands, style); move to `.claude/rules/<scope>.md` with `paths:` (subsystem rules); move to `docs/<topic>.md` (architecture diagrams, deployment runbooks, API tables, schema DDL); move to issue tracker (TODOs, known gaps); delete (already-in-the-repo content). Concrete walk-through pattern abstracted from real-use feedback. Documents the four asset-class strategies the configurator uses on retrofit + the manual vs `/retrofit` resolution paths. Linked from `00-overview.md` and the README's Use section.
- **README**: new "Resolving staged conflicts" subsection points at `/retrofit` (guided) and `docs/09-retrofit-guide.md` (manual + triage discipline).

With Tier 3 shipped, the full retrofit-safety roadmap is complete. v1.2.0 is ready to cut.

### Added
- **Retrofit safety — Tier 2 (PR 2c): CLAUDE.md merge-append strategy.** New `--claude-md={append|skip|overwrite}` flag (default `append`). On a collision, the configurator finds its three value-add sections in the generated CLAUDE.md (`## Working with Claude (collaboration patterns)`, `## Claude Code behavior rules`, `## Token efficiency rules`), and appends any that are missing from your existing CLAUDE.md to the bottom. Your existing content is preserved verbatim. Idempotent on re-runs: if all sections are already present, the configurator leaves your CLAUDE.md untouched and reports `"all value-add sections already present; left untouched"`. `skip` and `overwrite` modes match the corresponding `--on-collision` semantics for users who want different behavior. Detection is by exact heading match — same-heading-different-content is treated as "user has it" and skipped. New helpers in `configure.py`: `VALUEADD_HEADINGS`, `_extract_section`, `_append_missing_valueadd_sections`, `apply_claudemd_strategy`. Verified: append preserves user's "## My custom architecture" / "## Known gotchas" content untouched; re-running on a project already with all sections produces no-op + idempotent message; skip stages to `.claude-retrofit/incoming/CLAUDE.md`; overwrite backs up + replaces. With this PR, the Tier 2 trio (PR 2a structured deep-merge / PR 2b file collision strategies / PR 2c CLAUDE.md append) is complete; the only remaining piece of the retrofit roadmap is Tier 3 (`/retrofit` skill that walks REPORT.md interactively).

### Added
- **Retrofit safety — Tier 2 (PR 2b): file-collision strategies + retrofit report.** Non-structured assets (skills, agents, rules, hooks, `CLAUDE.md`) now follow a configurable strategy via `--on-collision={skip|rename|overwrite}`. Default is **skip**: your version stays untouched, ours is staged at `.claude-retrofit/incoming/<original-path>` so you can diff-and-merge manually. `rename` installs ours as a `-cc`-suffixed sibling so both coexist (`review` and `review-cc`). `overwrite` replaces yours with `.bak-<ts>`. After scaffold, a new `[ COLLISIONS ]` block summarizes what happened, and `.claude-retrofit/REPORT.md` records the full per-file picture with diff coordinates. New helpers in `configure.py`: `collision_renamed_target`, `apply_file_collision_strategy`, `write_retrofit_report`.
- **`--on-collision=MODE` flag** added; documented in README Flags + Use sections.

### Changed
- **Tier 1 abort behavior removed.** Default is now non-destructive: structured assets deep-merge, file-based assets follow `--on-collision=skip`. The `[ RETROFIT WARNINGS ]` block + exit-2 path that PR #6 introduced is gone — replaced by the new `[ MERGED ]` and `[ COLLISIONS ]` informational blocks. `--force` keeps its kill-switch role: skip both the merge AND the collision strategy; overwrite-with-backup like the pre-Tier-2 behavior. Removed dead `check_retrofit_state` helper that the abort path used.
- **`CLAUDE.md` collisions currently route through `skip`** (yours preserved, ours staged at `.claude-retrofit/incoming/CLAUDE.md`). PR 2c will upgrade to merge-append.
