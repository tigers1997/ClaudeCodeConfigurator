...
### Fixed
- **`/verify-setup` audits four new scaffold-vs-git bootstrap concerns.** `templates/commands/verify-setup/SKILL.md` already covered CLAUDE.md size, rule scoping, schema URL, MCP overhead, hook weight, local settings discipline, and orphan primitives (checks #1–#7) — but had no visibility into whether the configurator's output had actually been integrated into a real git repo, so all four issues surfaced by PRs #44 + #45 + #46 were invisible to the self-audit. Adds: **#8 Repo URL placeholder** (flags `[TODO:` or the pre-v2.4 literal `git@github.com:user/repo.git` still in CLAUDE.md), **#9 Claude Code gitignore block** (checks for the `# --- Claude Code ---` sentinel in `.gitignore`; suggests `cc-configure --retrofit` when missing), **#10 Nested `.git/` discipline** (`find -maxdepth 4 -name .git` then confirms each nested repo's parent is in `.gitignore`; covers vendored upstreams / fork wrappers), **#11 Scaffold committed** (only runs when a repo exists at root; checks `git ls-files --error-unmatch CLAUDE.md .claude/settings.json`). Frontmatter `allowed-tools` gains `Bash(find:*) Bash(git:*)` so the skill can actually run the new checks at session time. Sample output in the SKILL.md and `Suggested next actions` block updated to show the new rows. New `test/verify-setup/test-bootstrap-checks-present.sh` fixture pins the four headers and the two new tool entries against accidental regressions in future SKILL.md edits.
- **CLAUDE.md ships `### Repo bootstrap` guidance + `[ NEXT STEPS ]` nudges a missing `git init`.** Sibling gap to PRs #44 + #45 — both were *symptoms* of the same omission: the configurator scaffolds a project but never tells Claude (or the human) what the scaffold-time contract is. Symptom in the dogfood: after running cc-configure, the user `git init`'d, wrote a one-line `.gitignore` (losing the `# --- Claude Code ---` block the configurator had appended), committed only the spec, and left `.claude/` + `.claude-config.json` untracked. No signal anywhere in the output to course-correct. Two changes: (1) `templates/core/CLAUDE.md` gains a `### Repo bootstrap` subsection under `## HOW > ### Git workflow` codifying what to commit (`CLAUDE.md`, `.claude/`, `.mcp*.json`, `claude-ctx`, source), what to gitignore (`settings.local.json`, `.claude/logs/`, transient state files, `.claude-config.json`), the rule "preserve the Claude Code .gitignore block if you rewrite the file (or rerun `cc-configure --retrofit`)," and a nested-clone note. (2) `configure.py` extends the `[ NEXT STEPS ]` builder with a fourth trigger — when the target dir lacks `.git/`, the user sees a one-line nudge with the bootstrap incantation (`git init -b main && git add . && git commit -m '...'`). Suppressed under `--dry-run` since nothing's written. New `test/repo-bootstrap/` fixtures cover both arms (no-git → nudge fires, with-git → silent, dry-run → silent; subsection rendered with all three bullets).
- **`repo_url` default no longer leaks a fake URL into `CLAUDE.md`.** The form schema's `repo_url` field defaulted to the literal string `git@github.com:user/repo.git`, which flowed verbatim into `CLAUDE.md` (line: `**Repo:** {{repo_url}}`) and `.claude-config.json` for any user who accepted the default or ran `--yes`/`--persona` non-interactively. Surfaced by dogfooding `cc-configure` on a project in the brainstorming/planning phase (no remote set yet) — the obviously-fake URL got committed alongside the spec. The schema default is now empty; a new `normalize_conditional_placeholders()` step stamps a `[TODO:]` placeholder when `repo_url` is empty *or* matches the legacy literal (the second case keeps saved v2.3.x configs auto-upgrading on re-run). `[ PLACEHOLDERS ]` and `[ NEXT STEPS ]` now both fire on the field, so the user sees a one-line nudge to fill it in. The interactive `--quick` prompt gains an inline example (`Repo URL (optional, e.g. git@github.com:owner/repo.git) [...]:`) since the bracket no longer carries the format hint. New `test/repo-url-placeholder/` fixtures cover (a) empty default → `[TODO:]`, (b) legacy literal upgraded, (c) explicit URL passes through unchanged.
- **Stop hook skips silently when the stack manifest hasn't landed yet.** `templates/git-workflow/hooks/stop-run-checks.sh` already skipped when the first binary of a check command wasn't on PATH (so `uv typecheck` is silent on a box without `uv`). It did *not* skip when the binary existed but the project hadn't been scaffolded yet — so a project still in the brainstorming/planning phase with `cmd_test = "pnpm test"` would run `pnpm test` from a directory with no `package.json` every Stop and report `[stop-check] test: FAIL (exit 1)` to the next turn as additionalContext. Surfaced by dogfooding `cc-configure` on a project where the spec was being written before any code existed. New `manifest_for()` helper maps the first binary to its stack's manifest file (`package.json` for pnpm/npm/yarn/bun, `pyproject.toml` for uv/poetry/pip, `Cargo.toml` for cargo, `go.mod` for go, `Gemfile` for bundle/gem, `pom.xml` for mvn, `build.gradle` for gradle); tools not in the map (tsc, pytest, ruff, eslint, prettier, …) have no guard and run as before. POSIX `case` keeps the helper portable to macOS bash 3.2. New `test/stop-run-checks/test-manifest-guard.sh` exercises both arms via a fake `pnpm` that records invocation in a marker file (jq-independent so it runs on any box). **Behavior change for existing users:** users on the pre-fix hook only saw noise if (a) their tooling was installed but (b) the project root was missing a manifest — a rare state on mature codebases. Anyone hitting this gets quieter Stop output after they re-scaffold or copy the new hook in.
- **`.claude-config.json` now gitignored by default.** Added to `templates/core/.gitignore.append`. Surfaced by dogfooding `cc-configure` on an existing project: the configurator's own `/verify-setup` skill (check #6) flagged `.claude-config.json` as a working-state file that clutters diffs when tracked. Now matches the skill's recommendation out of the box. Inline comment in the gitignore explains that teams who want to share configurator selections across the repo can remove the line.
- **`token-efficiency` pro-tier rules folded into `CLAUDE.md`, dropping the `paths: ["**"]` antipattern.** The pro-tier `templates/token-efficiency/pro/_efficiency-core.md` shipped with `paths: ["**"]` (always-loaded) and was deployed to `.claude/rules/_efficiency-core.md` — exactly the shape the configurator's own `/verify-setup` skill (check #2 "Path-scoped rules") tells users not to keep: *"any rule whose `paths:` is `"**"` (matches everything) — should be in `CLAUDE.md` instead."* Surfaced by dogfooding `cc-configure` on an existing project. The prose body (Reading files / Running bash / Reset rhythm / Planning / Subagents / Inline bash in skills) now folds into the existing `## Token efficiency rules` section of `CLAUDE.md` under H3 subheads, alongside the form-driven `eff_*` bullets that already lived there; the standalone rule file is gone. `config_schema.py` drops the file from `extraPaths.pro` (the pro-tier `truncate-bash-output.sh` hook is unaffected). Two persona snapshots (`solo-experienced`, `small-team`) lose the file; the other three were on basic tier and never had it. New `test/efficiency-placeholder/` fixtures cover both tier paths in CI. **Retrofit note:** users re-running `cc-configure` over an existing v2 install with `tier=pro` will keep the stale `.claude/rules/_efficiency-core.md` on disk (the configurator doesn't auto-prune); `/verify-setup` continues to flag it as the discovery path.

### Changed
- **`settings.local.json.example` hints at MCP-token durability.** Adds a `// env` comment-key in `templates/core/dot-claude/settings.local.json.example` documenting that the `env` block is the durable home for MCP auth tokens (`SONATYPE_TOKEN` for the `security-auditor` agent, `GITHUB_TOKEN` for `mcp_github`). Surfaced by dogfooding `cc-configure` on an existing project: the `[ ENV WARNINGS ]` preflight already catches absent tokens, but users had no signpost to a durable location once they did set them — adding to a shell rc is the brittle status quo. Comment-key follows the existing `// sandbox` / top-level `//` pattern in the configurator. Pure docs / example change; no scaffold-time behavior change.
- **Bumped `CLAUDE_CODE_COMPAT.tested_up_to` from `2.1.128` → `2.1.132`.** Four CC releases came out (2.1.129, 2.1.130, 2.1.131, 2.1.132). One configurator-territory addition held pending schemastore: `skillOverrides` settings.json key (CC 2.1.129; values `off` / `user-invocable-only` / `name-only` to suppress always-on skill descriptions from context). Schemastore re-checked 2026-05-06, key not yet present — same unblock pattern as `prUrlTemplate` / `deniedDomains` (PRs #17/#18); next re-check 2026-05-13. Other items: 2.1.129 added an `experimental:` wrapper requirement for `themes`/`monitors` plugin-manifest entries (folded into the existing themes-module backlog note); 2.1.132 exposed `CLAUDE_CODE_SESSION_ID` in the env of Bash subprocesses spawned by the tool, matching the `session_id` already passed to hooks (no template change — worth a one-paragraph mention in `docs/03-commands-and-hooks.md` for hook authors, tracked in `07-backlog.md`). No new hook events or MCP/agent/skill frontmatter fields in configurator territory. Users on 2.1.129–132 no longer see the "newer than tested range" `[ VERSION WARNINGS ]` block. README Requirements line updated to match.
